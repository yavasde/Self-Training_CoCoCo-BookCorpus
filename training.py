...
an additional test set (BookCorpus).
...
probability = 0.9
predication_type = "Food" #or Event
...
BookCorpus_test = ...  # load the test data
BookCorpus = ...  # load the Book Corpus 
...
# Test the classifier on BookCorpus test set
test_classifier(clf, BookCorpus_test)

# Self-training loop starts here. Turn this to a loop:

# Apply the classifier to BookCorpus corpus and check the classifier probability and select the sentences based on that
...
# Test the classifier on BookCorpus test set
test_classifier(new_clf, BookCorpus_test)
